...
Mat determineFontImg ()
{
	...
	int h,w;
	cout << "how big is your canvas? (row col)(maybe 1000*1000?)\n";
	cin >> h >>w;

	Mat scrn1(h,w,CV_8UC3,Scalar(0,0,0));
	Mat scrn2(h,w,CV_8UC3,Scalar(0,0,0));
	Mat output(h,w,CV_8UC3,Scalar(0,0,0));
	back1 = scrn1;
	back2 = scrn2;
	Mat strImg;
...
{
	VideoCapture inputVideo( location + source );;
	...
	for (num_frame=0; num_frame>-1; num_frame++) {
		...
		temp_save_dir = location + num2string(num_frame) + ".jpg";
